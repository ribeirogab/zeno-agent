...
- **Sandboxed execution.** The agent runs inside a Docker container with no shell or filesystem access of its own — capabilities flow exclusively through connector MCP subprocesses spawned by the worker. The container has no host filesystem access beyond mounted volumes (`workspace`, `AGENTS.md` read-only, `knowledge/` read-only).
...
- **CLI is the only mutation surface.** All configuration and state changes (profiles, connectors, crons, skills, capabilities, backend credentials, channel setup) happen through the `zeno` CLI. The dashboard is read-only: it renders state, history, logs, and metrics — it never writes. This keeps a single, scriptable, auditable control plane and avoids two-source-of-truth drift between CLI and web. Migration of existing dashboard-write features is incremental, but no new mutation may be added to the dashboard. See `[[rules/cli-only-mutations]]`.
