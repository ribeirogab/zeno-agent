...
- **CLI origin bypass.** The CLI sends `X-Zeno-Origin: cli` header on every request (via the shared `ApiClient`). The mutation gate honors this header — when present, the gate skips the 403 and lets the request proceed. Without this bypass the rework would defeat itself: the CLI talks to the API the same way the dashboard does, so a blanket `cli` mode would also block the CLI. Trust model: the API binds 127.0.0.1 only, single-user; no cross-origin reach. The header value is checked literal (`=== 'cli'`), so typos and spoof attempts (`'CLI'`, `'dashboard'`, etc.) still get blocked.
