...
- [ ] `apps/worker/src/channels/slack/adapter.ts` `send` method signature matches the interface; its body branches on `message.attachments?.length` to route between `client.files.uploadV2` (with attachments) and `client.chat.postMessage` (without). Returned `messageRef` for the uploadV2 path is the `id` of the first uploaded file (not a posted-message ts — `files.uploadV2` does not surface one). Extraction handles both the nested wrapper shape `result.files[0].files[0].id` and the flat shape `result.files[0].id`. If neither yields a file id (defensive: SDK shape changes), throw `Error('files.uploadV2 returned no file id')`. Verified against `@slack/web-api@7.15.1` whose `filesUploadV2` returns `{ok, files: Array<completeUploadExternalResponse>}` where each entry is `{ok, files: [{id, title, ...}]}`.
...
- [ ] Unit test `SlackChannel.send: messageRef from files.uploadV2 is the first uploaded file's id (nested shape)` mocks `client.files.uploadV2` to return `{ok: true, files: [{ok: true, files: [{id: 'F1'}]}]}` and asserts the returned `messageRef === 'F1'`.
- [ ] Unit test `SlackChannel.send: falls back to a flat file shape when the SDK does not nest` mocks the SDK to return `{ok: true, files: [{id: 'F-FLAT'}]}` and asserts `messageRef === 'F-FLAT'`. Defends against SDK version drift between nested/flat shapes.
- [ ] Unit test `SlackChannel.send: files.uploadV2 returning no file id throws "files.uploadV2 returned no file id"` mocks `{ok: true, files: [{}]}` and asserts the promise rejects with that exact error message.
