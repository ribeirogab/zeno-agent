...
description: Clone repos using bare clones + git worktrees, develop changes, deliver via Pull Requests. Use this skill whenever the user asks you to clone, code, fix, edit, refactor, implement a feature, or open a PR on any repository. Covers branch naming, quality gate discovery, commit conventions, and PR delivery rules.
---
...
This is a **default Zeno skill** — it ships with the agent and applies to any profile. Profile-specific skills (e.g. `fn-code-review`) layer on top of this baseline.
...
- **Language:** match the repo's existing style (read `git log --oneline -20` to learn it). If unclear, default to English. Some teams (FN profile) use PT-BR — profile skills set that override.
...
- Read project docs (`AGENTS.md`, `CLAUDE.md`) before touching any code.
- Run the quality gate before every commit. Broken CI = your fault.
- Your commits are invisible — no agent attribution, ever.
