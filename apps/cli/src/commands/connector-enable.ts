...
type EnableClient = Pick<ApiClient, 'get' | 'patch'>;

/**
 * Resolve the connector by slug-or-id, then PATCH /:id/toggle if it is not
 * already enabled. The toggle endpoint is a direct write (200), so no command
 * polling is required.
 *
 * Spec 2026-05-08-connectors-cli-first-design Task 11.
 */
export async function runConnectorEnable(
  client: EnableClient,
  args: EnableArgs,
  ...
  if (detail.status === 'enabled') {
    print(ok(`${args.target} already enabled`));
    return;
  }
  await client.patch(`/api/connectors/${detail.id}/toggle`);
  print(ok(`${args.target} enabled`));
}
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    await runCommand(() => runConnectorEnable(client, { target }, (line) => console.log(line)));
  },
});
