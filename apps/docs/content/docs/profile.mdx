...
A **profile** is one Zeno instance — its own container, dashboard, master key, AGENTS.md operating manual, and credentials. You can have many side by side (`personal`, `work`, etc.); each one is independent.
...
The CLI allocates a host port (`6101+`), generates a master key, and scaffolds `~/.zeno/profiles/personal/AGENTS.md` and `.env` from the templates under `templates/profile/`.
...
```bash
$EDITOR ~/.zeno/profiles/personal/AGENTS.md
```

Fill in the per-instance operating manual: inviolable rules, which skills to invoke and when, channel conventions, language defaults. The agent reads this file on every turn (it is part of the cached system prompt).
