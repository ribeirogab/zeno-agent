...
| Edit agent/profile files from the Settings UI | Operator reports "I want to tweak SOUL.md without opening an editor" | Needs a write endpoint + re-check of the watcher not looping. Separate spec. |
...
| Idea | Trigger to promote | Notes |
|---|---|---|
| ~~GitHub App~~ | ~~Shipped~~ | Done — `acme-bot` GitHub App with per-org installation tokens, auto-rotation, and git identity via `config.yaml`. |
...
| `loadMcpConfig` duplicated between worker and api | `apps/worker/src/agent/mcp.ts` + `apps/api/src/lib/mcp-snapshot.ts` | Extract to a shared package when a third consumer appears. |
