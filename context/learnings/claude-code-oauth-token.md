...
To authenticate the Claude Agent SDK without `ANTHROPIC_API_KEY`, use a `CLAUDE_CODE_OAUTH_TOKEN`. This is how Wesker uses the user's Claude Pro/Max subscription flat-rate plan instead of pay-per-token API billing — aligning with the spec's decision to avoid an API key.

## Context

The spec explicitly requires OAuth auth, not API key. Discovery revealed two paths: (a) subprocess `claude -p` which reads OAuth from `~/.claude/` automatically, or (b) Claude Agent SDK which reads `CLAUDE_CODE_OAUTH_TOKEN` from env. Wesker uses (b) per [[claude-agent-sdk-typescript]].

Note: Anthropic's formal policy restricts OAuth-based auth for third-party products; the `CLAUDE_CODE_OAUTH_TOKEN` env var is documented in community demos ([weidwonder/claude_agent_sdk_oauth_demo](https://github.com/weidwonder/claude_agent_sdk_oauth_demo)) and requires an active Pro/Max subscription. For a personal agent (Wesker's scope), this is acceptable. For a multi-tenant product, prefer API key.
...
**Expiration / refresh:** Not documented. The demo README advises re-running `claude setup-token` if auth fails. Wesker's backend should classify auth failures as `kind: "auth_expired"` and surface a re-login instruction in Slack.
