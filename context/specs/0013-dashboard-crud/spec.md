...
- `enqueue(input: { type, payload?, correlationId }): Command` — inserts with `status='pending'`
- `claimPending(limit: number): Command[]` — atomic UPDATE-RETURNING that moves up to N rows from `pending` → `processing`. In SQLite: wrap in a transaction (`BEGIN; SELECT...; UPDATE WHERE id IN (...); SELECT...; COMMIT;`) — only one worker ever runs, so the race window is theoretical, but we code it correctly anyway.
- `finish(id, status: 'success' | 'failed', result?): void` — sets `completed_at`, `result`.
- `recent(limit: number): Command[]` — for future admin view (not used in Phase B UI).
- `sweepStuck(): number` — sets `status='failed'`, `result='{"error":"worker_restarted"}'` on any `processing` row. Called once on worker boot.
...
export class CommandsPoller {
  ...
  start(): void {
    // On boot: reclaim any row left in 'processing' from a prior run
...
The API has no direct access to runtime state of slack/backend inside the worker; those come from re-reading `/api/health` internally (or duplicated env lookups where applicable). For `mcpServers` the API re-reads `profile/mcp.json` via the same `loadMcpConfig` helper used by the worker (moved to a shared place — see packaging note below).
...
| EDIT | `context/specs/0013-dashboard-crud/spec.md` — this file |
...
1. **Operator pauses a cron from the UI.**
   ...
   - Toast: "pausando…".
   - Within 2s the status pill turns to "Paused".
...
3. `curl -X POST -H 'Cookie: zeno_auth=…' http://localhost:3000/api/crons/<id>/pause` returns `204`; within 2s the next `GET /api/crons/<id>` shows `enabled: false`.
...
| Dashboard-initiated `cron_delete` races with a concurrent tick of the same cron (cron was about to fire) | The runner reads enabled crons each tick. If the delete lands between the tick's `due()` and the `runner.execute()`, the run still completes against a deleted cron id — `markRun` will fail silently. Acceptable for v1; could add a "cron still exists" check before run, but it's YAGNI. |
...
| `loadMcpConfig` code is currently inside `apps/worker/src/agent/mcp.ts` and now needed by `apps/api` — duplication or cross-app import | Decide during implementation: (a) move to a new `packages/mcp-config` package, or (b) duplicate the ~60 lines. Recommend (a). Either way: zero impact on public API. |
