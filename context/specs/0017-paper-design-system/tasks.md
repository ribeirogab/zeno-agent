...
- Quality-gate just to be safe: `pnpm run quality-gate`.

- [ ] **Step 2: Push**

```bash
git push -u origin feat/paper-design-system
```

- [ ] **Step 3: Open PR via `/open-pr`**

Title: `docs(ui): Paper design system — catalog, registry, and governance rule`

Body covers:
- Paper file organized into 5 named sections (Foundations, Primitives, Patterns, Feature components, Pages)
- New `packages/ui/DESIGN.md` registry linking every rendered `.tsx` to a Paper frame
- New rule `context/rules/ui-in-paper.md` (severity: important)
- Zero runtime impact (docs + design only)

Do NOT merge — user reviews Paper frames in parallel with the registry.
