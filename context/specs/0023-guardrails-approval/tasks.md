...
- [ ] Verify current Slack app manifest includes `im:write`, `chat:write`, `reactions:read`, `reactions:write`. If missing, note in plan and surface to user before proceeding.
  ...
  ```
  Check files: `infra/`, `context/specs/0001-slack-zeno-mvp/`, and any `manifest.yaml` or `slack-app.json`. If manifest isn't versioned, document current Slack app scopes as discovered.
...
- [ ] Commit:
  ```bash
  git add context/learnings/ context/specs/0023-guardrails-approval/plan.md
...
- [ ] In `context/specs/0023-guardrails-approval/spec.md`, update frontmatter:
  ```yaml
  status: shipped
  shipped: 2026-MM-DD
  ```
