...
- [ ] **Item 7:** spec at `context/specs/0030-ui-primitive-lift/spec.md` has Approved status from spec-document-reviewer (verified before this plan was written) and the user has approved the spec.
...
The PR body should reference `context/specs/0030-ui-primitive-lift/spec.md`, summarize the 5 phases, and link to the closed open-questions resolutions documented in this plan.

---

## Done

When all checkboxes above are ticked, the spec is implemented. Branch is push-ready and the spec status can be updated from `draft` to `shipped` in `context/specs/0030-ui-primitive-lift/spec.md` frontmatter (and the `shipped: null` field set to today's ISO date).
