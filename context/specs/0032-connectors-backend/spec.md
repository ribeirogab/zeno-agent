...
| The `mcp__<slug>__<tool>` regex misparses slugs containing underscores (e.g., `fn-scrum`, `google-drive`) | Slugs are kebab-case (no underscores allowed by spec 0029 §Slug collision). The regex assumes that. Slug shape is enforced at **two layers**: (1) the migration adds a CHECK that combines SQLite-valid GLOB clauses to require kebab-case (`slug GLOB '[a-z0-9]*' AND slug NOT GLOB '*[^a-z0-9-]*' AND length(slug) >= 1`); (2) the writer (`ConnectorRepo.create` and the API layer) validates via a Zod schema (`/^[a-z0-9][a-z0-9-]*$/`) before insert so error messages are clear. SQLite GLOB does not support `+`/`?` quantifiers, so the CHECK uses two complementary GLOB tests rather than a single regex-style pattern. |
