...
- **Single fixture stdio MCP** (echo server) shared across all Phase A scenarios. Three tools: `read_echo`, `write_echo`, `interactive_echo`. Failure modes via env vars (`FIXTURE_FAIL=auth`, `FIXTURE_FAIL=spawn`, `FIXTURE_FAIL=timeout`).
