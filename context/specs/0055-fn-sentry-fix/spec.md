...
- **Auto-resolve stale path:**
  ...
  - Action: Sentry MCP `update_issue(issueId, status='resolved', note='Resolved in <commit_sha>: <commit_subject>. No new events since.')` — `status` and `note` are parameters of Sentry MCP's `update_issue` tool (not invented; verify against the Sentry MCP server's tool schema at runtime; if `note` is not the field name, use whatever the schema exposes for "resolution comment").
...
5. **Blast radius bounded** — anticipated fix touches ≤5 files / ≤100 LOC, does NOT touch DB schema, migrations, auth/permission code, public APIs, or cross-package refactors.
...
2. **Regression test FIRST.** Write a test that:
   ...
   - Mocks ALL externals: HTTP calls, DB, file system, third-party SDKs. 100% local.
