...
- [ ] **A.11.1** Open `context/specs/0055-fn-sentry-fix/spec.md` side-by-side with `profiles/fn/skills/fn-sentry-fix/SKILL.md`. Walk the spec's Decisions table + Success Criteria + Security Rules + Cost Caps. For each, confirm the skill body has the corresponding instruction.
...
- [ ] **D.1.1** Diff branch against main:
  ...
  ```
  Expected: 1-3 commits (spec + skill + LICENSE + maybe E2E iteration commits) under `context/specs/0055-fn-sentry-fix/` + `profiles/fn/skills/fn-sentry-fix/`.
