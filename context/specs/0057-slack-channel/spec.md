...
| 4 | disabled (any secret state) | missing | **HARD ERROR**: `Slack credentials not configured — install Slack channel via dashboard or set SLACK_APP_TOKEN/SLACK_BOT_TOKEN in profile .env`. |
...
- **MODIFIED** worker-side command handler `connector_create` (in `apps/worker/src/commands/handlers.ts`). The API route at `apps/api/src/routes/connectors.ts:837` enqueues a `connector_create` command; the worker handler is what actually calls `ConnectorRepo.create()`. The spec REQUIRES that the handler forward the `kind` field from the command payload to the repo's `create()` call. Without this, the API accepts `kind: 'channel'` but the row lands in the DB with the column DEFAULT `'mcp'` — silent bug that fails acceptance criterion "channel row inserted with `kind='channel'`". The command payload schema (likely a Zod schema next to the handler) must include `kind`. Integration test required: end-to-end POST → enqueue → handler → DB row → assert `kind='channel'`. (Unit tests that mock `ConnectorRepo.create()` would PASS while the integration is broken — that's the trap.)
