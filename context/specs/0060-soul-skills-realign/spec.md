...
- **Code-review output drifts** — multi-sentence praise, missing review URL, wrong @-mention target, no template adherence. Defeats the entire purpose of `fn-code-review`.
...
- **E2E test on the live `zeno-fn` container is the contract acceptance.** Open a real test PR in `AcmeBooks/ecommerce-frontend` (PR title prefixed `[zeno-test]` to mark it disposable), `@zeno-agent` mention in `#C0EXAMPLE000` Slack channel, validate the response shape against Templates A/B/C/D + the Slack reply shape from `fn-code-review/SKILL.md`. Container is already running on port 3001 (built from main).
- **No skill content changes.** SKILL.md files in `profiles/fn/skills/` are out of scope. Only `agent/SOUL.md`, `apps/worker/src/agent/system-prompt.ts`, and `apps/worker/src/agent/backends/claude-code.ts` change in this spec.
...
1. **Operator mentions @zeno-agent on a PR review request.** Slack message: `@reviewer @zeno-agent <github.com/Org/repo/pull/N>` + PR description. Agent's system prompt now contains a skill listing block including `fn-code-review: Review pull requests on GitHub following Flávia Nasser's git workflow...`. The description matches the trigger, the agent reads `~/.claude/skills/fn-code-review/SKILL.md`, follows Template A/B/C/D, submits review with `gh api` + `gh pr review`, and the auto-posted Slack reply matches `<@USER_ID> <verdict> · <counts> · <review-url>`.

2. **Operator mentions @zeno-agent on a Sentry issue.** Same path: `fn-sentry-fix` is in the listing, description matches, agent reads + follows the skill.
...
**Phase D — E2E live test against zeno-fn container (the contract acceptance):**
...
- [ ] After the agent's reply is posted, validate against `fn-code-review/SKILL.md` contracts:
...
context/specs/0060-soul-skills-realign/
├── spec.md                                         # this file
├── plan.md                                         # next file
└── tasks.md                                        # next file
```
...
TODAY:
  ...
  → Agent has no awareness of fn-code-review existing
...
AFTER:
  ...
  → Agent sees `fn-code-review: Review pull requests on GitHub...` in system prompt
...
- [ ] Branch built into image: `cd /Users/operator/www/octocat/zeno-agent && PROFILE=fn pnpm run docker:build && PROFILE=fn pnpm run docker:up`.
...
- **Skill listing budget might truncate `fn-code-review`'s description.** Subagent investigation (sdk.d.ts) cited `skillListingMaxDescChars` (default 1536) and `skillListingBudgetFraction` (default 0.01). A re-grep at spec-review time could not confirm those exact field names in the public Options type — they may live in `Settings` or be private SDK internals. Pragmatic stance: `fn-code-review`'s description is ~600 chars, well under any reasonable per-skill cap. If E2E shows the listing got truncated, file a follow-up to investigate budget knobs.
...
- **`fn-code-review` SKILL uses `gh api` (Bash) for review submission, not MCP github tools.** Means the connector-skill injection path doesn't fire for it (would only fire on `mcp__github-app-fnlivros__*` calls). The fix here (skill listing in system prompt) is the ONLY reliable path for `fn-code-review` — by surfacing the skill in the agent's awareness and letting auto-discovery + Read take over. This is why the SOUL realignment + preset shape combo is critical, not optional.
...
- Spec 0052 (skills reintroduced as DB playbooks): `context/specs/0052-skills/spec.md`
- The bug-trigger SKILL.md: `profiles/fn/skills/fn-code-review/SKILL.md`
- The bug observation: worker logs from `zeno-fn-agent-1` on 2026-04-30 reviewing `https://github.com/AcmeBooks/infrastructure/pull/12` — see analysis in this conversation's thread.
