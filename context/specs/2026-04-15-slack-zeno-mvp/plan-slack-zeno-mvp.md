...
**For this spec:** `[[spec-slack-zeno-mvp]]`
...
- `docker-compose.yml` — Single `zeno-agent` service, `workspace` volume, `USER.md` bind-mount (read-only)
...
Execution plan is in `[[tasks-slack-zeno-mvp]]`.
