...
> **For agentic workers:** Steps live in `[[tasks]]` and use checkbox (`- [ ]`) syntax. Branch: `feat/dashboard-visual-rebuild` (newly created from `main`). Each phase ends with a quality gate (lint + typecheck + test + build on `apps/dashboard`) and a manual visual verification side-by-side with `apps/design`.
...
**Spec reference:** `[[spec]]` (`context/specs/2026-04-26-dashboard-visual-rebuild/spec.md`).
