...
2. **First-time setup, custom remote.** Operator opens `/connectors`, clicks `Add custom`. Picks `Remote URL`, names it `Acme Scrum`, pastes the URL, expands `Advanced` and pastes the OAuth Client Secret. Clicks `Test connection`, sees `✓ 8 tools (5 read · 3 write)`. Clicks `Add`. Connector lands as `enabled`.
