...
- **Data preservation across upgrade.** Existing dev profiles must be deleted and recreated post-merge. No legacy bootstrap shim, no schema-drift verification, no `schema_migrations` backfill for legacy DBs.
...
- **Idempotent boot:** `runRuntimeMigrations` and `seedDefaultConnectors` must be safe to invoke on every boot. Both worker and api invoke them; SQLite serializes via the wrapping transaction in `applyMigrations`.
- **Reuse `packages/db/src/shared/`:** the host package already exposes `openSqlite`/`closeSqlite` (with the canonical pragmas) and `applyMigrations`/`loadMigrations` (against a `schema_migrations` table). The runtime subpath reuses these helpers verbatim — drizzle-kit produces the `.sql` files; the existing shared runner applies them. We do **not** introduce drizzle's own `migrate()` runner or its `__drizzle_migrations` table.
...
3. **Worker boot, fresh profile.** `runtime.db` does not exist. `openRuntimeDatabase` creates the file. `runRuntimeMigrations` applies `0000_baseline.sql` and inserts a row in `schema_migrations`. `seedDefaultConnectors` upserts catalog defaults (Playwright). Repos instantiate. Slack channel ready to accept mentions.
4. **Worker boot, profile previously created on this build.** Same code path as #3; `migrate()` no-ops because `schema_migrations` lists the baseline. `seedDefaultConnectors` upserts (idempotent — zero net inserts). Repos instantiate.
...
- [ ] A vitest test asserts `runRuntimeMigrations` is idempotent: invoking it twice on a fresh `:memory:` DB yields exactly one row in `schema_migrations` and zero diffs in `sqlite_master` between the two invocations.
...
- [ ] Manual smoke E2E recorded in PR description: commands run, output excerpts pasted: `zeno profile delete dev-test`, `zeno profile create dev-test --owner test`, `zeno start dev-test`, `@zeno hello` Slack mention with bot reply, `sqlite3 ~/.zeno/profiles/dev-test/runtime.db ".tables"` showing `schema_migrations` populated and all 17 runtime tables present (`sessions`, `logs`, `commands`, `crons`, `cron_runs`, `cron_connectors`, `cron_skills`, `connectors`, `connector_secrets`, `connector_tool_permissions`, `connector_invocations`, `connector_apps`, `connector_skills`, `skills`, `agent_capabilities`, `backend_credentials`, `backend_settings`).
...
| Concurrent `applyMigrations` from worker and api on first boot. | The shared runner wraps each migration in a `db.transaction(...)`, which serializes against `journal_mode=WAL` + `busy_timeout=5000`. The second caller observes the version recorded in `schema_migrations` and skips. Verified by acceptance test. |
