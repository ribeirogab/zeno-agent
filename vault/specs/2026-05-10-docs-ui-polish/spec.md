...
- [ ] `app/[[...slug]]/page.tsx` passes an `editOnGithub` prop to `<DocsPage>` whose URL pattern resolves to `https://github.com/ribeirogab/zeno-agent/edit/main/apps/docs/content/docs/<file>` for the current page's source file.
- [ ] The "Edit on GitHub" link renders in the page footer area provided by Fumadocs `DocsPage`. Clicking it opens GitHub's editor on the correct file (verify on `/install` → `install.mdx`).
...
| Edit-on-GitHub URL construction depends on `page.file.path` returning a path relative to the MDX collection root, not the OS-absolute path. Wrong base will produce 404s on GitHub's edit endpoint. | Helper extracts the basename + extension and prepends `apps/docs/content/docs/`. Unit-testable: given `page.file.path === 'install.mdx'`, helper returns `https://github.com/ribeirogab/zeno-agent/edit/main/apps/docs/content/docs/install.mdx`. Confirm on `/install` before merging. |
