...
Tests live under `apps/worker/tests/`, NOT colocated with `src/`. Verified during planning:

- **Wrapper tests:** existing file is `apps/worker/tests/agent/wrap-context.test.ts` (100 lines, vitest, imports `wrapWithSlackContext` from `@/agent/core`). The plan updates this file in place: rename the symbol under test (`wrapWithSlackContext` → `wrapWithChannelContext`), update existing assertions, add parity assertions (`toBe`-based byte equality), add non-slack-with-attachments cases. File name `wrap-context.test.ts` is already aligned with the new function name.
- **Slack adapter tests:** no `adapter.test.ts` exists. Existing slack test files in `apps/worker/tests/channels/slack/`: `format.test.ts`, `wait-reaction.test.ts`, `normalize.test.ts`, `files.test.ts`. The plan creates a new `apps/worker/tests/channels/slack/adapter.test.ts` for the cleanup-behavior cases. Mocking pattern follows `wait-reaction.test.ts` (vi.mock of `@slack/bolt`, fake `App` with listener registry, dispatch via `__listeners.get('app_mention')`). File-system writes/reads use real `tmpdir()` paths (pattern from `files.test.ts`); `fetch` mocked via `vi.spyOn(globalThis, 'fetch')`.
- **Existing assertion to preserve:** the test `returns plain text for non-slack platforms` in `wrap-context.test.ts:21-24` currently asserts `wrapWithSlackContext({platform: 'discord', text: 'hi'}) === 'hi'`. This MUST remain green for the renamed function — the no-attachments case still returns text verbatim. A NEW test must cover `{platform: 'discord', attachments: [...]}` to lock in the de-Slack-gating behavior.
